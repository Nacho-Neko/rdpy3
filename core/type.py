...
from rdpy3.core.error import InvalidExpectedDataException, InvalidSize, CallPureVirtualFuntion, InvalidValue
...
class Type(object):
    ...
    def read(self, s):
        ...
        if old != self:
            ...
            # breakpoint()
            # rollback read value
            # if old.value ==  b'test_tpkt_layer_recv': breakpoint() # s.pos = 0
            s.pos -= sizeof(self)
            raise InvalidExpectedDataException(
                "%s const value expected %s != %s" % (self.__class__, old.value, self.value))
    ...
    def __sizeof__(self):
        ...
        """
        raise CallPureVirtualFuntion("%s:%s defined by interface %s" % (
            self.__class__, "__sizeof__", "Type"))
...
class CallableValue(object):
    ...
    def __setValue__(self, value):
        ...
        """
        def value_callable(): return value
        if callable(value):
...
class SimpleType(Type, CallableValue):
    ...
    def __init__(self, structFormat, typeSize, signed, value, conditional=lambda: True, optional=False, constant=False):
        ...
        Type.__init__(self, conditional=conditional,
    ...
    def __read__(self, s):
        ...
        if s.dataLen() < self._typeSize:
            raise InvalidSize(
                "Stream is too small to read expected SimpleType")
        self.value = struct.unpack(
            self._structFormat, s.read(self._typeSize))[0]
    ...
    def __read__(self, s):
        ...
        for name in self._typeName:
            try:
                ...
                # log.debug("readLen : {} type {} sizeof {}".format(readLen,self.__dict__[name],sizeof(self.__dict__[name])))
                # read is ok but read out of bound
                if not self._readLen is None and readLen > self._readLen.value:
                    log.debug("readLen: {}  padding: {}".format(readLen,self._readLen.value))
                    # roll back
                    s.pos -= sizeof(self.__dict__[name])
                    # and notify if not optional
                    if not self.__dict__[name]._optional:
                        raise InvalidSize(
                            "Impossible to read type %s : read length is too small" % (self.__class__))
        ...
        if not self._readLen is None and readLen < self._readLen.value:
            log.debug("readLen: {}  padding: {}".format(readLen,self._readLen.value))
            log.debug("Still have correct data in packet %s, read %s bytes as padding" % (
                self.__class__, self._readLen.value - readLen))
...
class UInt8(SimpleType):
    ...
    def __init__(self, value=0, conditional=lambda: True, optional=False, constant=False):
        ...
        SimpleType.__init__(self, "B", 1, False, value,
                            conditional=conditional, optional=optional, constant=constant)
...
class SInt16Le(SimpleType):
    ...
    def __init__(self, value=0, conditional=lambda: True, optional=False, constant=False):
        ...
        @param constant:   Check if object value doesn't change after read operation
        """
        SimpleType.__init__(self, "<h", 2, True, value,
                            conditional=conditional, optional=optional, constant=constant)
...
class UInt24Be(SimpleType):
    ...
    def __read__(self, s):
        ...
        """
        self.value = struct.unpack(
            self._structFormat, b'\x00' + s.read(self._typeSize))[0]
...
class UInt24Le(SimpleType):
    ...
    def __read__(self, s):
        ...
        """
        self.value = struct.unpack(
            self._structFormat, s.read(self._typeSize) + b'\x00')[0]
...
class String(Type, CallableValue):
    ...
    def __init__(self, value="", readLen=None, conditional=lambda: True, optional=False, constant=False, str=False, until=None):
        """
    ...
    def __read__(self, s):
        ...
        if self._readLen is None:
            ...
            # log.debug("test :"  + str(self._readLen.value))
        if self._unicode:
...
def encodeUnicode(s):
    ...
    """
    return "".join([c + b"\x00" for c in s]) + b"\x00\x00"
...
def decodeUnicode(value):
    ...
    lenght = len(value) - 2
    while i < lenght:
        char = struct.unpack("<H", value[i:i+2])[0]
        result += chr(char)
        i += 2
    #s.read(i)
    return result
...
class Stream(BytesIO):
    ...
    def readType(self, value):
        ...
        #current_position = self.tell()  # 获取当前位置
        #remaining_bytes = self.read()
        #将字节数据转换为十六进制表示
        #hex_output = remaining_bytes.hex()
        # self.seek(current_position)
        # log.debug("seek : {} type : {} hex : {}".format(current_position,value,hex_output))
