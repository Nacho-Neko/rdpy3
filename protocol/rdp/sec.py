...
from rdpy3.core.type import CompositeType, CallableValue, Stream, UInt32Le, UInt16Le, String, sizeof, UInt8
...
def saltedHash(inputData, salt, salt1, salt2):
    ...
    sha1Digest.update(inputData.encode('utf-8'))
...
def updateKey(initialKey, currentKey, method):
    ...
    if method == gcc.EncryptionMethod.ENCRYPTION_FLAG_40BIT:
        ...
        rc4 = ARC4.new(tempKey128[:8])
        return gen40bits(rc4.encrypt(tempKey128[:8]))
    
    elif method == gcc.EncryptionMethod.ENCRYPTION_FLAG_56BIT:
        tempKey128 = tempKey(initialKey[:8], currentKey[:8])
        rc4 = ARC4.new(tempKey128[:8])
        return gen56bits(rc4.encrypt(tempKey128[:8]))
    
    elif method == gcc.EncryptionMethod.ENCRYPTION_FLAG_128BIT:
        tempKey128 = tempKey(initialKey, currentKey)
        rc4 = ARC4.new(tempKey128)
        return rc4.encrypt(tempKey128)
...
class RDPInfo(CompositeType):
    ...
    def __init__(self, extendedInfoConditional):
        ...
        #microsoft domain
        self.domain = String(readLen = CallableValue(lambda:self.cbDomain.value + 2), str = True)
        self.userName = String(readLen = CallableValue(lambda:self.cbUserName.value + 2), str = True)
        self.password = String(readLen = CallableValue(lambda:self.cbPassword.value + 2), str = True)
        #shell execute at start of session
        self.alternateShell = String(readLen = CallableValue(lambda:self.cbAlternateShell.value + 2), str = True)
        #working directory for session
        self.workingDir = String(readLen = CallableValue(lambda:self.cbWorkingDir.value + 2), str = True)
...
class RDPExtendedInfo(CompositeType):
    ...
    def __init__(self, conditional):
        ...
        self.clientAddress = String(readLen = self.cbClientAddress, str = True)
        self.cbClientDir = UInt16Le(lambda:sizeof(self.clientDir))
        self.clientDir = String(readLen = self.cbClientDir, str = True)
        #TODO make tiomezone
...
class SecLayer(LayerAutomata, IStreamSender, tpkt.IFastPathListener, tpkt.IFastPathSender, mcs.IGCCConfig):
    ...
    def __init__(self, presentation):
        ...
        #current rc4 tab
        self._decryptRc4 : ARC4Cipher = None
        self._encryptRc4 : ARC4Cipher = None
    ...
    def readEncryptedPayload(self, s, saltedMacGeneration):
        ...
        if self._nbDecryptedPacket == 4096:
            ...
            self._decryptRc4 = ARC4.new(self._currentDecrytKey)
        ...
        decrypted = self._decryptRc4.encrypt(encryptedPayload.value)
        # decrypted = rc4.crypt(self._decryptRc4, encryptedPayload.value)

        #ckeck signature
        data = macData(self._macKey, decrypted)[:8]
        if not saltedMacGeneration and data != signature.value:
            raise InvalidExpectedDataException("bad signature")
        
        saltedData = macSaltedData(self._macKey, decrypted, self._nbDecryptedPacket)[:8]
        if saltedMacGeneration and saltedData != signature.value:
            raise InvalidExpectedDataException("bad signature")
    ...
    def writeEncryptedPayload(self, data, saltedMacGeneration):
        ...
        if self._nbEncryptedPacket == 4096:
            ...
            self._currentEncryptKey = updateKey(    self._initialEncryptKey, self._currentEncryptKey, 
                                                    self.getGCCServerSettings().SC_SECURITY.encryptionMethod.value)
            self._encryptRc4 = ARC4.new(self._currentEncryptKey)
            self._nbEncryptedPacket = 0
        ...
        if saltedMacGeneration:
            return (String(macSaltedData(self._macKey, s.getvalue(), self._nbEncryptedPacket - 1)[:8]), String(self._encryptRc4.encrypt(s.getvalue())))
        else:
            return (String(macData(self._macKey, s.getvalue())[:8]), String(self._encryptRc4.encrypt(s.getvalue())))
...
class Server(SecLayer):
    ...
    def getCertificate(self):
        ...
        certificate.PublicKeyBlob.modulus.value = rsa.int2bytes(self._rsaPublicKey.n,sizeof(certificate.PublicKeyBlob.modulus))[::-1]
