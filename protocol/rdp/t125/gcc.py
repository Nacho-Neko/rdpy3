...
from rdpy3.core.type import UInt16Be, UInt32Be, UInt8, UInt16Le, UInt32Le, CompositeType, CallableValue, String, Stream, sizeof, FactoryType, ArrayType
from . import per, mcs
from rdpy3.core.error import InvalidExpectedDataException
from rdpy3.core import log
from rdpy3.security import x509
        ...
        def DataBlockFactory():
            ...
            for c in [ClientCoreData, ClientSecurityData, ClientNetworkData, ServerCoreData, ServerNetworkData, ServerSecurityData]:
                if self.type.value == c._TYPE_:
                    log.debug(" hex {} ,type {} ,length  {}".format(hex(self.type.value) , hex(c._TYPE_), self.length.value))
                    if self.type.value == MessageType.CS_CORE:
                        return c(readLen = self.length - 8)
...
class ClientCoreData(CompositeType):
    ...
    def __init__(self, readLen = None):
        ...
        self.rdpVersion = UInt32Le(Version.RDP_VERSION_5_PLUS) # 0d 00 08 00
        self.desktopWidth = UInt16Le(1280) # 0a a0  // 1440
        self.desktopHeight = UInt16Le(800) # 05 01
        self.colorDepth = UInt16Le(ColorDepth.RNS_UD_COLOR_8BPP) # 03 ca
        self.sasSequence = UInt16Le(Sequence.RNS_UD_SAS_DEL) # 04 08
        self.kbdLayout = UInt32Le(KeyboardLayout.US) # 61 00 00 00
        self.clientBuild = UInt32Le(3790) # 4a 00 00 00
        self.clientName = String(("rdpy" + "\x00" * 12).encode("utf-16le"), readLen = CallableValue(32), str = True)
        # 44 00 45 00 53 00 4b 00 54 00 4f 00 50 00 2d 00 31 00 51 00 48 00 4e 00 49 00 51 00 35 00 00 00
        self.keyboardType = UInt32Le(KeyboardType.IBM_101_102_KEYS) #  00 04 00 00 
        self.keyboardSubType = UInt32Le(0) # 00 00 00 00
        self.keyboardFnKeys = UInt32Le(12) # 0c 00 00 00
        ...
        self.postBeta2ColorDepth = UInt16Le(ColorDepth.RNS_UD_COLOR_8BPP, optional = True) # 00 00
        self.clientProductId = UInt16Le(1, optional = True) # 00 00
        self.serialNumber = UInt32Le(0, optional = True) # 00 00 00 00
        self.highColorDepth = UInt16Le(HighColor.HIGH_COLOR_24BPP, optional = True) # 18 00 
        ...
        # 0f 00 
        self.earlyCapabilityFlags = UInt16Le(CapabilityFlags.RNS_UD_CS_SUPPORT_ERRINFO_PDU, optional = True) #  af 07 
        self.clientDigProductId = String("b\x00"*64, readLen = CallableValue(64), optional = True)
        # 61 00 38 00 37 00 35 00 63 00 36 00 36 00 2d 00 33 00 39 00 38 00 66 00 2d 00 34 00 61 00 32 00 34 00 2d 00 61 00 37 00 65 00 62 00 2d 00 33 00 30 00 34 00 62 00 61 00 63 00 35 00 00 00
        self.connectionType = UInt8(optional = True) # 07
        self.pad1octet = UInt8(optional = True) #  00 
        self.serverSelectedProtocol = UInt32Le(optional = True) # 00 00 02 58
...
class ServerSecurityData(CompositeType):
    ...
    def __init__(self, readLen = None):
        ...
        self.serverRandom = String(readLen=lambda: self.serverRandomLen.value, conditional=lambda: not(self.encryptionMethod.value == 0 and self.encryptionLevel.value == 0))
...
class ProprietaryServerCertificate(CompositeType):
    ...
    def computeSignatureHash(self):
        ...
        computehash = md5Digest.digest() + b"\x00" + (b"\xff" * 45) + b"\x01"
        return computehash
    ...
    def sign(self):
        ...
        """
        _TERMINAL_SERVICES_PRIVATE_EXPONENT_ =  ProprietaryServerCertificate._TERMINAL_SERVICES_PRIVATE_EXPONENT_[::-1]
        _TERMINAL_SERVICES_MODULUS_ =  ProprietaryServerCertificate._TERMINAL_SERVICES_MODULUS_[::-1]
        privateKey = rsa.PrivateKey(d = _TERMINAL_SERVICES_PRIVATE_EXPONENT_, n = _TERMINAL_SERVICES_MODULUS_)
        self.SignatureBlob.value = rsa.sign(self.computeSignatureHash()[::-1], privateKey)[::-1]
...
class RSAPublicKey(CompositeType):
    ...
    def __init__(self, readLen):
        ...
        self.datalen = UInt32Le(lambda:((old_div(self.bitlen.value, 8)) - 1))
...
class ServerNetworkData(CompositeType):
    ...
    def __init__(self, readLen = None):
        ...
        self.channelIdArray = ArrayType(UInt16Le, readLen=lambda: self.channelCount.value)
...
def readConferenceCreateRequest(s : Stream):
    """
    ...
    #remaining_bytes = s.read()
    # 将字节数据转换为十六进制表示
    #hex_output = remaining_bytes.hex()
    #print("length : {} , hex : {}".format(length, hex_output))
...
def writeConferenceCreateResponse(serverData):
    ...
    mv = serverDataStream.getbuffer()
    for byte in mv:
        print(f"{byte:02x}", end=" ")

    
    choice1 : UInt8 =  per.writeChoice(0)
    objectIdentifier : tuple[UInt8,UInt8,UInt8,UInt8,UInt8] = per.writeObjectIdentifier(t124_02_98_oid)
    length : (UInt16Be | UInt8) = per.writeLength(len(serverDataStream.getvalue()) + 14)
    choice2 : UInt8 = per.writeChoice(0x14)
    integer16 : UInt16Be = per.writeInteger16(0x79F3, 1001),
    integer1 : tuple[UInt16Be | UInt8, UInt8] | tuple[UInt16Be | UInt8, UInt16Be] | tuple[UInt16Be | UInt8, UInt32Be] = per.writeInteger(1)
    enumerates : UInt8 = per.writeEnumerates(0)
    numberOfSet : UInt8 = per.writeNumberOfSet(1)
    choice3 : UInt8 = per.writeChoice(0xc0)
    h221_sc_keyStream : tuple[UInt16Be | UInt8, tuple] = per.writeOctetStream(h221_sc_key, 4)
    serverData : tuple[UInt16Be | UInt8, tuple] = per.writeOctetStream(serverDataStream.getvalue())
    
    log.debug(f"length : {length.value}")

    return (choice1, objectIdentifier,
            length,choice2,
            integer16, integer1, enumerates,
            numberOfSet, choice3,
            h221_sc_keyStream, serverData)