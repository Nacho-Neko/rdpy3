...
from rdpy3.core.layer import LayerAutomata, IStreamSender, Layer
from rdpy3.core.type import sizeof, Stream, UInt8, UInt16Le, String
from rdpy3.core.error import InvalidExpectedDataException, InvalidValue, InvalidSize, CallPureVirtualFuntion
...
class Server(MCSLayer):
    ...
    def recvChannelJoinRequest(self, data):
        ...
        channelId = per.readInteger16(data)
        #actually algo support virtual channel but rdpy3 have no virtual channel
        confirm = 0 if channelId in list(self._channels.keys()) or channelId == self._userId else 1
    ...
    def sendConnectResponse(self):
        ...
        tmp = (ber.writeEnumerated(0), ber.writeInteger(0), self.writeDomainParams(22, 3, 0, 0xfff8), 
               ber.writeOctetstring(ccReqStream.getvalue()))
        data = (ber.writeApplicationTag(Message.MCS_TYPE_CONNECT_RESPONSE, sizeof(tmp)), tmp)
