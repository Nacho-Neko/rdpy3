...
class ProductInformation(CompositeType):
    ...
    def __init__(self):
        ...
        self.pbCompanyName = String("Microsoft Corporation", readLen = self.cbCompanyName, str = True)
        self.cbProductId = UInt32Le(lambda:sizeof(self.pbProductId))
        #may contain "A02" from microsoft license server
        self.pbProductId = String("A02", readLen = self.cbProductId, str = True)
...
class ClientNewLicenseRequest(CompositeType):
    ...
    @summary:  Send by client to ask new license for client.
                rdpy3 doesn'support license reuse, need it in futur version
...
class LicenseManager(object):
    ...
    def sendClientChallengeResponse(self, platformChallenge):
        ...
        rc4 = ARC4.new(self._licenseKey)
        serverChallenge = rc4.encrypt(serverEncryptedChallenge)
        ...
        message.encryptedHWID.blobData.value = rc4.encrypt(hwid)
