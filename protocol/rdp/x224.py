...
class Client(X224Layer):
    ...
    def recvConnectionConfirm(self, data):
        ...
        if self._selectedProtocol in [ Protocols.PROTOCOL_HYBRID_EX ]:
            raise InvalidExpectedDataException("rdpy3 doesn't support PROTOCOL_HYBRID_EX security Layer")
...
class ExpiringObject:
    def __init__(self, s_ipaddres, expiry_time):
        self.s_ipaddres = s_ipaddres
...
class ExpiringList:
    def __init__(self):
        self.items = []
        #self.cleanup_thread = threading.Thread(target=self.cleanup_expired_loop)
        #self.cleanup_thread.start()

    def has(self, ip_address):
        for item in self.items:
            if item.s_ipaddres == ip_address:
                current_time = time.time()
    ...
    def append(self, ip_address, expiry_time):
        expiry_time = time.time() + expiry_time
        self.items.append(ExpiringObject(ip_address, expiry_time))
    ...
    def cleanup_expired(self):
        ...
        for expired_item in expired_items:
            self.items.remove(expired_item)

    def cleanup_expired_loop(self):
        while True:
            time.sleep(1)
...
class Server(X224Layer):
    ...
    def sendConnectionConfirm(self):
        ...
        client = self._transport.getDescriptor().client
        ip_address, port = client
        if expiring_list.has(ip_address):
            self._transport.send(message)
            message.protocolNeg.selectedProtocol.value = 0
            expiring_list.update(ip_address)
            #port_black.add_condition(ip_address,port,port+1)
        else:
            #log.error("{}: {}\n".format(ip_address, port))
            log.error("ip_address : {} not in".format(ip_address, port))
            expiring_list.append(ip_address,10)
            message.protocolNeg.selectedProtocol.value = 2
            self._transport.send(message)
